...
{
  #ifndef IOT_CONFIG_USE_X509_CERT
  Serial.println(IOT_CONFIG_DEVICE_KEY);
...
static void getTelemetryPayload(az_span payload, az_span* out_payload)
{
  ...
  payload = az_span_copy(
      payload, AZ_SPAN_FROM_STR("{ \"water_level\": "));
  (void)az_span_u32toa(payload, nMoistureLevelVal, &payload);
...
void waterLevel()
{
    nMoistureLevelVal = analogRead(nMoistureLevelIn);
    Serial.print("MoistureLevel: ");
    Serial.println(nMoistureLevelIn);
    bIsWet = !(digitalRead(nWaterDigitalIn));
    Serial.print("IsWet");
    Serial.println(bIsWet);
}
